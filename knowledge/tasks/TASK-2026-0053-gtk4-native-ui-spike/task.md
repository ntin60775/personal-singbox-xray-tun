...
- восстановленная подзадача после архивного smoke: `subtasks/TASK-2026-0053.6-gtk4-compact-ux-defect-pass/`
...
Задача закрыта как уже пройденный этап реализации. Функциональная часть `TASK-2026-0053` собрана: после `TASK-2026-0053.4` с готовым `Log` подзадача `TASK-2026-0053.5` добавила отдельный launcher для ручного тестирования `GTK4` native UI.

Позднее архивный ручной smoke от `2026-04-10` всё же был проведён на живом display и выявил отдельное продолжение по UX. Реализация `TASK-2026-0053` из-за этого не переоткрывалась: замечания вынесены в восстановленную подзадачу `TASK-2026-0053.6`, а сам этап остаётся завершённым как функциональный spike.
...
Дополнительных обязательных ручных проверок по этой задаче не осталось: исторический ручной smoke от `2026-04-10` уже зафиксирован и вынесен в отдельную подзадачу `TASK-2026-0053.6`.

Если потребуется практический rollout native UI или решение о переводе launcher-а по умолчанию, это нужно продолжать отдельной задачей или подзадачей и уже там проходить живой desktop/runtime smoke.
...
После закрытия функционального spike в archive был дополнительно проведён ручной smoke от `2026-04-10`, который выявил UX-дефекты geometry и визуального контракта. Эти замечания теперь восстановлены как отдельная подзадача `TASK-2026-0053.6`, поэтому исходная задача остаётся завершённой, а rollout-риск прозрачно вынесен в отдельный контур.
