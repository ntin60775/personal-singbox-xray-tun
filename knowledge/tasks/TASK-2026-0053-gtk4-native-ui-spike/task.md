...
Задача остаётся в активной реализации, но первый этап уже закрыт: `TASK-2026-0053.1` завершила рабочий `GTK4` shell, tray/fallback-контур, минимальное окно настроек и совместимый формат shell-настроек в `gui_settings.json`. Следующие этапы остаются за service-layer, реальным наполнением экранов и backend-проводкой действий, а итоговый ручной smoke теперь должен поддерживаться единым списком только в рамках этой общей задачи.
