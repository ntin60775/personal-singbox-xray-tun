...
Выполнен третий точечный UX-pass поверх компактного shell-контракта: на `Dashboard` появился master-`CTA`, который позволяет сразу запустить подключение по последнему выбранному узлу без перехода на другие вкладки, а оставшиеся пользовательские строки `GTK4`-оболочки и связанных backend-сообщений переведены на русский. После синхронизации knowledge и повторного регрессионного прогона подзадача остаётся в стадии проверки до живого GTK smoke на доступном display.
