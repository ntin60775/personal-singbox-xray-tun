...
По решению пользователя `TASK-2026-0053` закрывается без этого smoke. Список ниже остаётся reference-контуром для возможной будущей отдельной задачи по rollout native UI; именно здесь по-прежнему собраны runtime/tray/manual сценарии закрытой `TASK-2026-0053.2`.
