...
- `install-or-update-bundle-dir.sh` — установка или обновление bundle в отдельном каталоге
...
Это критично для провайдеров, которые отдают рабочий payload только `Xray`-клиентам. В публичной версии репозитория используется обезличенный example URL подписки, но сам механизм рассчитан на реальные провайдерские endpoint-ы, которые отдают рабочие `VLESS`-ноды только совместимым клиентам.
...
Это именно sync bundle-каталога, а не установка системных зависимостей. Если каталог развёрнут на новой машине впервые, после копирования при необходимости отдельно запусти:
