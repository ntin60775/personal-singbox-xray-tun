...
Скрипт создаёт `~/.local/share/applications/subvost-xray-tun.desktop` с абсолютными путями к текущему bundle. Ярлык открывает GUI без раннего root-запроса; `pkexec` появляется только при действиях `Старт`, `Стоп` и `Диагностика`. Если каталог bundle перемещён, installer ярлыка нужно запустить повторно.
